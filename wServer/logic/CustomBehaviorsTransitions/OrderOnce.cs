...
namespace wServer.logic.CustomBehaviorsTransitions
{
    class OrderOnce : Behavior
    {
        ...
        {
            _range = range;
            _children = BehaviorDb.InitGameData.IdToObjectType[children];
            _targetStateName = targetState;
        }

        private static State FindState(State state, RealmTime time, string name)
        {
            if (state.Name == name)
                return state;

            return state.States
                .Select(i => FindState(i, time, name))
        ...
        protected override void OnStateEntry(Entity host, RealmTime time, ref object state)
        {
            if (_targetState == null)
                _targetState = FindState(host.Manager.Behaviors.Definitions[_children].Item1, time, _targetStateName);
